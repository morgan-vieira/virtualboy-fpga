...
| `vip-stereo`      | `vip_stereo`                  | Different art per eye: the left is a dim field with a bright 32x32 block top-left, the right a brighter field with one top-right, and a bright bar 32 pixels apart between them. Walk Core Settings > Stereo Mode: the two 2D settings must differ, Side-by-Side puts the left picture on the left, the interleaves dither the two together, Cyberscope quarter-turns both, Anaglyph shows the bars 32 apart in two colours. Identical 2D settings mean one eye read twice; a swapped field means crossed eyes. |
