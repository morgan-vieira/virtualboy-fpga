...
| `vip-stereo`      | `vip_stereo`                  | Different art per eye: the left is a dim field with a bright 32x32 block top-left, the right a brighter field with one top-right, and a bright bar 32 pixels apart between them. Walk all seventeen Core Settings > Stereo Mode rows: the eight 2D rows show the left eye alone in the colour they name, the five Anaglyph rows show both bars in two colours, Side By Side puts the dim picture on the left, CyberScope quarter-turns both with black margins, and the two interlaces dither them together. Two identical Side By Side halves mean one eye read twice; a swapped field means crossed eyes. |
...
| `vip-compose`     | `vip` world composition       | Diagnostic Overlay on: the centre square fills and the status cells read 0x600D; a failing check's number shows instead. Covers world sizes, base rounding, >8 maps, overplane, END, overlap, palettes, and register byte-write mangling. Mednafen freezes at check 12 by design (beetle-vb byte writes and base OR).                                                                                     |
| `vip-objx`        | `vip` object conformance      | Diagnostic Overlay on: square fills, cells read 0x600D. Covers group boundaries and cycling, JLON/JRON, parallax, flips, negative JY, clipping, and end-below-start wrap; check 9 legitimately overruns a frame. Mednafen freezes at check 10 by design (beetle-vb saturates the group counter and gates object eyes on world LON/RON).                                                                   |
| `vip-hbaff`       | `vip` h-bias and affine       | Diagnostic Overlay on: square fills, cells read 0x600D. Covers per-eye h-bias offsets, the odd-base alias, affine identity and both parallax signs, signed wrap, affine overplane, 16-bit parameter wrap, and unaligned-base rejection. Mednafen freezes at check 8 by design (beetle-vb masks the parameter base).                                                                                       |
| `vip-sched`       | `vip` scheduling              | Diagnostic Overlay on: square fills, cells read 0x600D; checks 8-9 wait through heavy scenes, allow a few seconds. Covers XPEN/XPBSY/XPEND, FRMCYC cadence, buffer ownership, SBOUT/SBHIT, XPRST, overtime, and DPRST. Mednafen freezes at check 7 by design (beetle-vb never raises SBHIT or OVERTIME and swaps on XPRST).                                                                               |
| `vip-dpctrl`      | `vip` display control         | Diagnostic Overlay on: cells read 0x600D with the square hollow, over a red field brightest at the left fading right; every ~2 s the field alternates with a uniformly bright one (LOCK holding CTA at 0xFA). Mednafen freezes at check 4 by design (beetle-vb hides FCLK and fixes CTA).                                                                                                                 |
...
status halfword and a bottom row for PC bits 16-1. It is drawn in one eye's
384x224 coordinates, so it looks the same as it always has in the 384x224
stereo modes and follows the picture in the others: both halves side by side,
both panels quarter-turned under Cyberscope, one readable copy under either
line interleave, and at zero parallax in anaglyph. The overlay is **off by
