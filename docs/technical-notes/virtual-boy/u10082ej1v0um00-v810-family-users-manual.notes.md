# Notes: V810 FAMILY 32-BIT MICROPROCESSOR — ARCHITECTURE
...
- TKCW fields: bits 31-9 RFU (fixed to 0); bit 8 OTM (Operand Trap Mask); bit 7 FIT (Floating Invalid Operation Trap Enable); bit 6 FZT (Floating-Zero Divide Trap Enable); bit 5 FVT (Floating-Overflow Trap Enable); bit 4 FUT (Floating-Underflow Trap Enable); bit 3 FPT (Floating-Precision Trap Enable); bit 2 RDI (Floating Rounding Control Bit for Integer Conversion); bits 1, 0 RD (Floating Rounding Control). [§2.2.6, p.13]
...
- CHCW fields: bits 31-8 = SA (Spill-Out Base Address); bits 31-20 = CEN (Clear Entry Number); bits 19-8 = CEC (Clear Entry Count); bits 7, 6 = RFU (fixed to 0); bit 5 = ICR (Instruction Cache Restore); bit 4 = ICD (Instruction Cache Dump); bits 3, 2 = RFU (fixed to 0); bit 1 = ICE (Instruction Cache Enable); bit 0 = ICC (Instruction Cache Clear). [§2.2.7, p.14]
...
- 0 = EIPC (Exception/Interrupt PC) — LDSR enabled, STSR enabled. [§2.2.9, p.15]
- 1 = EIPSW (Exception/Interrupt PSW) — LDSR enabled, STSR enabled. [§2.2.9, p.15]
- 2 = FEPC (Fatal Error PC) — LDSR enabled, STSR enabled. [§2.2.9, p.15]
- 3 = FEPSW (Fatal Error PSW) — LDSR enabled, STSR enabled. [§2.2.9, p.15]
- 4 = ECR (Exception Cause Register) — LDSR disabled, STSR enabled. [§2.2.9, p.15]
- 5 = PSW (Program Status Word) — LDSR enabled, STSR enabled. [§2.2.9, p.15]
- 6 = PIR (Processor ID Register) — LDSR disabled, STSR enabled. [§2.2.9, p.15]
- 7 = TKCW (TasK Control Word) — LDSR disabled, STSR enabled. [§2.2.9, p.15]
- 8–23 = Reserved. [§2.2.9, p.15]
- 24 = CHCW (CacHe Control Word) — LDSR enabled, STSR enabled. [§2.2.9, p.15]
- 25 = ADTRE (ADdress Trap Register for Execution) — LDSR enabled, STSR enabled. [§2.2.9, p.15]
- 26–31 = Reserved. [§2.2.9, p.15]
...
- Word data must be aligned at the word boundary (lower 2 address bits 0) and halfword data at the halfword boundary (lower 1 address bit 0). Unless aligned, the lower bit(s) — 2 bits for word data, 1 bit for halfword data — is automatically masked to 0 for access. [§3.2, p.20]
...
- Operand symbols: `reg1` general-purpose register (used as source register); `reg2` general-purpose register (mainly used as destination register); `imm5` 5-bit immediate; `imm16` 16-bit immediate; `disp9` 9-bit displacement; `disp16` 16-bit displacement; `disp26` 26-bit displacement; `regID` system register number; `vector adr` trap handler address corresponding to vector. [§5.3, p.39]
- Operation symbols: `<-` substitution; `||` bit connection; `GR [x]` general-purpose register x; `SR [x]` system register x; `sign-extend (x)` extends sign of x to word length; `zero-extend(x)` zero-extends x to word length; `converted (x)` converts type of x with rounding direction depending on TKCW; `truncate (x)` converts type of x with rounding direction 0; `Load-Memory (x, y)` reads data of size y from address x; `Store-Memory (x, y, z)` writes data y of size z to address x; `Input-Port (x, y)` reads data of size y from port address x; `Output-Port (x, y, z)` writes data y of size z to port address x; `adr` 32-bit unsigned address. [§5.3, p.39]
...
- **Table 5-9 Conditional Branch Instructions** (p.50) — BGT 1111 `((S xor OV) or Z) = 0` greater than signed; BGE 1110 `(S xor OV) = 0`; BLT 0110 `(S xor OV) = 1`; BLE 0111 `((S xor OV) or Z) = 1`; BH 1011 `(CY or Z) = 0`; BNL 1001 `CY = 0`; BL 0001 `CY = 1`; BNH 0011 `(CY or Z) = 1`; BE 0010 `Z = 1`; BNE 1010 `Z = 0`; BV 0000 `OV = 1`; BNV 1000 `OV = 0`; BN 0100 `S = 1`; BP 1100 `S = 0`; BC 0001 `CY = 1`; BNC 1001 `CY = 0`; BZ 0010 `Z = 1`; BNZ 1010 `Z = 0`; BR 0101 always (unconditional); NOP 1101 not always (does not branch). [Table 5-9, p.50]
...
- **Table 5-10 Condition Codes** (p.91) — 0000 V `OV = 1`; 1000 NV `OV = 0`; 0001 C/L `CY = 1`; 1001 NC/NL `CY = 0`; 0010 Z `Z = 1`; 1010 NZ `Z = 0`; 0011 NH `(CY or Z) = 1`; 1011 H `(CY or Z) = 0`; 0100 S/N `S = 1`; 1100 NS/P `S = 0`; 0101 T always 1; 1101 F always 0; 0110 LT `(S xor OV) = 1`; 1110 GE `(S xor OV) = 0`; 0111 LE `((S xor OV) or Z) = 1`; 1111 GT `((S xor OV) or Z) = 0`. [Table 5-10, p.91]
...
- Integer arithmetic / logical operation cycle counts: MOV, ADD, SUB, CMP, SHL, SHR (reg1, reg2) = 1; MUL = 13; DIV = 38; MULU = 13; DIVU = 36; OR = 1; MOV imm5, reg2 = 1; MOVEA imm16, reg1, reg2 = 1. [Table 5-11 (1/3), p.107]
...
- Floating-point instruction cycle counts: CVT.WS = 5–16; CVT.SW = 9–14; TRNC.SW = 8–14; CMPF.S = 7–10; ADDF.S = 9–28; SUBF.S = 12–28; MULF.S = 8–30; DIVF.S = 44. [Table 5-11 (3/3), p.109]
...
- Table 5-13 cycle counts by transfer type (1 word 32-bit bus / 1 word 16-bit bus / 2 word 32-bit bus / 2 word 16-bit bus / Nth word 32-bit bus / Nth word 16-bit bus): TYPE1 32 / 38 / 41 / 53 / `6N + 30` / `12N + 30`; TYPE2 32 / 38 / 42 / 54 / `6N + 31` / `12N + 31`; TYPE3 37 / 43 / 48 / 60 / `6N + 35` / `12N + 35`; TYPE4 43 / 49 / 49 / 61 / `6N + 36` / `6N + 36`; TYPE5 32 / 38 / 43 / 55 / `6N + 31` / `12N + 31`; TYPE6 14 / 20 / — / — ; TYPE7 37 / 43 / — / —. N is stated to be the number of words of memory space occupied by the source bit string (N is the last word of the source bit string). [Table 5-13, p.114]
...
- **Table 6-1 Exception Codes** (p.117), listing exception/interrupt, classification, exception code, handler address, and restore PC:
  - Reset — Interrupt — code FFF0 — handler FFFFFFF0H — restore PC per Note 2 (EIPC and FEPC are undefined).
  - NMI — Interrupt — code FFD0 — handler FFFFFFD0H — restore PC = next PC (Note 3).
  - Duplexed exception — Exception — code per Note 4 — handler FFFFFFD0H — restore PC = current PC.
  - Address trap — Exception — code FFC0 — handler FFFFFFC0H — restore PC = current PC.
  - Trap instruction (parameter is 0x1n) — Exception — code FFBn — handler FFFFFFB0H — restore PC = next PC.
  - Trap instruction (parameter is 0x0n) — Exception — code FFAn — handler FFFFFFA0H — restore PC = next PC.
  - Invalid instruction code — Exception — code FF90 — handler FFFFFF90H — restore PC = current PC.
  - Zero division — Exception — code FF80 — handler FFFFFF80H — restore PC = current PC.
  - FIV (floating-point invalid operation) — Exception — code FF70 — handler FFFFFF60H — restore PC = current PC.
  - FZD (floating-point zero division) — Exception — code FF68 — handler FFFFFF60H — restore PC = current PC.
  - FOV (floating-point overflow) — Exception — code FF64 — handler FFFFFF60H — restore PC = current PC.
  - FUD (floating-point underflow) — Exception (Note 5) — code FF62 — handler FFFFFF60H — restore PC = current PC.
  - FPR (floating-point precision degradation) — Exception (Note 5) — code FF61 — handler FFFFFF60H — restore PC = current PC.
  - FRO (floating-point reserved operand) — Exception — code FF60 — handler FFFFFF60H — restore PC = current PC.
  - INT level n (n = 0–15) — Interrupt — code FEn0 — handler FFFFFEn0H — restore PC = next PC (Note 3).
  [Table 6-1, p.117]
...
- Table A-1's page column gives the following locations for the full §5.3 descriptions: ADD 41, ADDF.S 42, ADDI 44, AND 45, ANDBSU 46, ANDI 47, ANDNBSU 48, Bcond 50, CAXI 51, CMP 53, CMPF.S 54, CVT.SW 55, CVT.WS 57, DIV 58, DIVF.S 59, DIVU 61, HALT 62, IN 63, JAL 64, JMP 65, JR 66, LD 67, LDSR 68, MOV 69, MOVBSU 70, MOVEA 71, MOVHI 72, MUL 73, MULF.S 74, MULU 76, NOT 77, NOTBSU 78, OR 79, ORBSU 80, ORI 81, ORNBSU 82, OUT 83, RETI 84, SAR 85, SCH0BSU/SCH0BSD 86, SCH1BSU/SCH1BSD 88, NOP 89, SETF 90, SHL 92, SHR 93, ST 94, STSR 95, SUB 96, SUBF.S 97, TRAP 99, TRNC.SW 101, XOR 103, XORBSU 104, XORI 105, XORNBSU 106. [Table A-1, pp.134–140]
...
- **Table B-2 Instruction Set** (p.142) maps 6-bit op codes to instruction and format: 000000 MOV reg1, reg2 (I); 000001 ADD reg1, reg2 (I); 000010 SUB reg1, reg2 (I); 000011 CMP reg1, reg2 (I); 000100 SHL reg1, reg2 (I); 000101 SHR reg1, reg2 (I); 000110 JMP [reg1] (I); 000111 SAR reg1, reg2 (I); 001000 MUL reg1, reg2 (I); 001001 DIV reg1, reg2 (I); 001010 MULU reg1, reg2 (I); 001011 DIVU reg1, reg2 (I); 001100 OR reg1, reg2 (I); 001101 AND reg1, reg2 (I); 001111 NOT reg1, reg2 (I). [Table B-2, p.142]
- Table B-2 continued: 010000 MOV imm5, reg2 (II); 010001 ADD imm5, reg2 (II); 010010 SETF imm5, reg2 (II); 010011 CMP imm5, reg2 (II); 010100 SHL imm5, reg2 (II); 010101 SHR imm5, reg2 (II); 010110 unassigned; 010111 SAR imm5, reg2 (II); 011000 TRAP vector (II); 011001 RETI (II); 011010 HALT (II); 011011 unassigned; 011100 LDSR reg2, regID (II); 011101 STSR regID, reg2 (II); 011110 unassigned; 011111 Bstr (II). [Table B-2, p.142]
- Table B-2 continued: `100$$$$` Bcond disp9 (III); 101000 MOVEA imm16, reg1, reg2 (V); 101001 ADDI imm16, reg1, reg2 (V); 101010 JR disp26 (IV); 101011 JAL disp26 (IV); 101100 ORI (V); 101101 ANDI (V); 101110 XORI (V); 101111 MOVHI (V). [Table B-2, p.142]
- Table B-2 continued: 110000 LD.B disp16 [reg1], reg2 (VI); 110001 LD.H (VI); 110010 unassigned; 110011 LD.W (VI); 110100 ST.B reg2, disp16 [reg1] (VI); 110101 ST.H (VI); 110110 unassigned; 110111 ST.W (VI); 111000 IN.B (VI); 111001 IN.H (VI); 111010 CAXI disp16 [reg1], reg2 (VI); 111011 IN.W (VI); 111100 OUT.B (VI); 111101 OUT.H (VI); 111110 Fpp reg1, reg2 (VII); 111111 OUT.W (VI). [Table B-2, p.142]

### APPENDIX C OP CODE MAP (p.143)

- Op code map (a) is indexed by bits 15..13 (rows 0–7) against bits 12..10 (columns 0–7): row 0 = MOV, ADD, SUB, CMP, SHL, SHR, JMP, SAR (Format I); row 1 = MUL, DIV, MULU, DIVU, OR, AND, XOR, NOT; row 2 = MOV, ADD, SETF, CMP, SHL, SHR, (blank), SAR (Format II); row 3 = TRAP, RETI, HALT, (blank), LDSR, STSR, (blank), Bstr; row 4 = Bcond at column 3 (Format III); row 5 = MOVEA, ADDI, JR, JAL, ORI, ANDI, XORI, MOVHI (Format IV/V); row 6 = LD.B, LD.H, (blank), LD.W, ST.B, ST.H, (blank), ST.W (Format VI); row 7 = IN.B, IN.H, CAXI, IN.W, OUT.B, OUT.H, Fpp, OUT.W (Format VI/VII). [Appendix C(a), p.143]
- Branch instruction condition-code map (b) is indexed by bit 12 (rows 0–1) against bits 11..9 (columns 0–7): row 0 = BV, BC/BL, BZ/BE, BNH, BN, BR, BLT, BLE; row 1 = BNV, BNC/BNL, BNZ/BNE, BH, BP, NOP, BGE, BGT. [Appendix C(b), p.143]
- Bit string manipulation sub-op code map (c) is indexed by bits 4..3 (rows 0–1) against bits 2..0 (columns 0–7): row 0 = SCH0BSU, SCH0BSD, SCH1BSU, SCH1BSD (columns 0–3, columns 4–7 blank); row 1 = ORBSU, ANDBSU, XORBSU, MOVBSU, ORNBSU, ANDNBSU, XORNBSU, NOTBSU. [Appendix C(c), p.143]
- Floating-point operation sub-op code map (d) is indexed by bits 31..29 (rows 0–7) against bits 28..26 (columns 0–7): row 0 = CMPF.S (column 0), CVT.WS (column 2), CVT.SW (column 3), ADDF.S (column 4), SUBF.S (column 5), MULF.S (column 6), DIVF.S (column 7); row 1 = TRNC.SW (column 3); rows 2–7 blank. [Appendix C(d), p.143]
...
- Word data must be aligned at the word boundary and halfword data at the halfword boundary; unaligned accesses have the lower address bit(s) automatically masked to 0. [§3.2, p.20]
