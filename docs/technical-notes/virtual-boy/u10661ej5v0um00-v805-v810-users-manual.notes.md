# Notes: V805™, V810™ 32/16, 32-BIT MICROPROCESSOR, HARDWARE volume (µPD70731, µPD70732)
...
A dash inside a table below reproduces the dash the manual prints in that cell. In the part-number and pin-status tables it means the entry does not apply. In the system register table its meaning is the manual's own legend, "Write disabled".
...
- The "Major Revisions in This Edition" table lists five changes [Major Revisions in This Edition]:
...
- Features listed for both parts. [§1.1, p.1]
...
- §3.1 lists seven items it covers for 32-bit bus mode. [§3.1, p.26]
...
- Under dynamic bus sizing, a byte read at operand address bits (1,0) = (0,0) or (1,0) takes B1, and at (0,1) or (1,1) takes B2. A halfword read at either (0,0) or (1,0) takes B2:B1. In all cases the data comes from the lower 16 bits of the data bus in bus cycle 1. [Figure 3-4(a)–(b), p.29]
...
- A bit string processes data by word (4 bytes) to increase processing speed. The end of the written data may not sit at a word boundary, meaning bits 1 and 0 of the last byte address in the destination bit string are not "11". In that case up to 3 bytes are excessively written, but data is not destroyed because the original value is written back unchanged. [Caution "Notes on bit string", §3.1.3, p.31]
...
- Maskable interrupt servicing. [§4.2.1(1)–(6), p.68]

  1. save the restore PC to EIPC
  2. Save the current PSW to EIPSW
  3. Write the exception code to the lower 16 bits of the ECR (EICC)
  4. Set the EP and ID bits of the PSW and clear the AE bit
  5. Set n + 1 into the I (I3 to I0) field of the PSW, or 15 if the accepted level is n = 15
  6. Jump to the handler address
...
The table pairs each pin with an I/O circuit type number and a recommended connection. As transcribed, the type numbers and connections are partially collapsed into single cells. The legible values are:

| Pin                                          | Circuit type | Recommended connection          |
| -------------------------------------------- | ------------ | ------------------------------- |
| D15 to D0 (V805 only), D31 to D0 (V810 only) | 5            | "Open"                          |
| A31 to A1                                    | 4            |                                 |
| READY                                        | 1            | "Connect to GND via a resistor" |
| HLDRQ                                        |              | "Connect to VDD via a resistor" |
| IC1                                          |              | "Open"                          |
| IC2                                          |              | "Connect to GND"                |
| IC3 (V810 only)                              |              | "Connect to VSS"                |

BE1/BE0 (V805 only), BE3 to BE0 (V810 only), SIZ16B, SZRQ and ICHEEN are listed without a legible individual value, and CLK is listed with the table's "not applicable" dash. Figure 2-1 (p.21) shows the I/O circuit of each type.

### Address, data length, byte enable and A1 in 32-bit bus mode (Table 3-1, p.27)
...
### Address, data length, byte enable and A1 in 16-bit bus fixed mode (Table 3-3, p.47)
...
### Machine fault cycle bus contents in 32-bit bus mode (Table 3-2, p.41)
...
### Machine fault cycle bus contents in 16-bit bus fixed mode (Table 3-4, p.58)
