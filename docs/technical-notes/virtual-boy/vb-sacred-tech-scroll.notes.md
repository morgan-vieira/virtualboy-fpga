...
- Component ranges. [Memory Map]

  | Range                     | Component                   |
  | ------------------------- | --------------------------- |
  | `0x00000000`–`0x00FFFFFF` | VIP                         |
  | `0x01000000`–`0x01FFFFFF` | VSU                         |
  ...
  | `0x08000000`–`0xFFFFFFFF` | mirroring of the memory map |
...
- Miscellaneous hardware registers. [Memory Map > Miscellaneous Hardware]

  | Address                   | Register | Owner    | Name                                           |
  | ------------------------- | -------- | -------- | ---------------------------------------------- |
  ...
  | `0x02000040`–`0x02FFFFFF` |          |          | mirroring of the hardware component memory map |
...
- Exception handlers occupy `0xFFFFFE00`–`0xFFFFFFFF`, 16 bytes each. [ROM Format > Exception Handlers]

  | Address                   | Handler                  |
  | ------------------------- | ------------------------ |
  | `0xFFFFFE00`              | game pad interrupt       |
  | `0xFFFFFE10`              | timer zero interrupt     |
  | `0xFFFFFE20`              | game pak interrupt       |
  | `0xFFFFFE30`              | communication interrupt  |
  | `0xFFFFFE40`              | VIP interrupt            |
  | `0xFFFFFE50`–`0xFFFFFEFF` | unused                   |
  | `0xFFFFFF00`–`0xFFFFFF5F` | unused                   |
  | `0xFFFFFF60`              | floating-point exception |
  | `0xFFFFFF70`–`0xFFFFFF7F` | unused                   |
  | `0xFFFFFF80`              | zero division exception  |
  | `0xFFFFFF90`              | illegal opcode exception |
  | `0xFFFFFFA0`              | `TRAP` (vector < 16)     |
  | `0xFFFFFFB0`              | `TRAP` (vector ≥ 16)     |
  | `0xFFFFFFC0`              | address trap             |
  | `0xFFFFFFD0`              | duplexed exception       |
...
- Specifications table. [CPU > Specifications]
...
- Program registers with instruction-level significance. [CPU > Register Set > Program Registers]
  ...
  | `r29`    | bit string destination word address / bits skipped in a search                                                                    |
  | `r30`    | upper 32 bits of integer multiplication, remainder of integer division, exchange value for `CAXI`, bit string source word address |
  | `r31`    | return address of `JAL`                                                                                                           |

- Compiler and assembler names. [CPU > Register Set > Program Registers]

  | Register | Alias | Name                                                                           |
  | -------- | ----- | ------------------------------------------------------------------------------ |
  | `r0`     |       | Zero Register                                                                  |
  | `r1`     |       | Assembler Reserved (for constructing 32-bit immediate data)                    |
  | `r2`     | `hp`  | Handler Stack Pointer                                                          |
  | `r3`     | `sp`  | Stack Pointer (full-stack convention, points at the most recently added value) |
...
- `CHCW` fields. [CPU > `CHCW`]

  | Field | Name                      | Access and meaning                                                               |
  | ----- | ------------------------- | -------------------------------------------------------------------------------- |
  | `CEN` | Clear Entry Number        | W, index of first cache entry to clear, read as zeroes                           |
  | `CEC` | Clear Entry Count         | W, read as zeroes                                                                |
  | `SA`  | Spill-Out Base Address    | W, higher 24 bits of the dump/restore address, lower 8 bits zero, read as zeroes |
  | `ICR` | Instruction Cache Restore | W                                                                                |
  | `ICD` | Instruction Cache Dump    | W                                                                                |
  | `ICE` | Instruction Cache Enable  | R/W                                                                              |
  | `ICC` | Instruction Cache Clear   | W                                                                                |
...
- `PSW` field meanings. All fields are R/W. [CPU > `PSW`]

  | Field | Name                      | Meaning                                                |
  | ----- | ------------------------- | ------------------------------------------------------ |
  | `I`   | Interrupt Level           | an interrupt whose level is less than `I` is masked    |
  | `NP`  | NMI Pending               | set during processing of a duplexed exception or reset |
  | `EP`  | Exception Pending         | set during processing of an exception or interrupt     |
  | `AE`  | Address Trap Enable       |                                                        |
  | `ID`  | Interrupt Disable         | if set, all interrupts are masked                      |
  | `FRO` | floating reserved operand |                                                        |
  | `FIV` | floating invalid          |                                                        |
  | `FZD` | floating zero divide      | set when `DIVF.S` is executed with a divisor of zero   |
  | `FOV` | floating overflow         |                                                        |
  | `FUD` | floating underflow        |                                                        |
  | `FPR` | floating precision        |                                                        |
  | `CY`  | carry                     |                                                        |
  | `OV`  | overflow                  |                                                        |
  | `S`   | sign                      |                                                        |
  | `Z`   | zero                      |                                                        |
...
- Dumped and restored tag format. [CPU > Instruction Cache]

  | Field                  | Bits  | Width | Note                                               |
  | ---------------------- | ----- | ----- | -------------------------------------------------- |
  | unused                 | 31–28 | 4     | "Tag memory in the cache is only 27 bits wide"     |
  | `NECRV` (NEC Reserved) | 27–23 | 5     | "The significance of this field is not documented" |
  | `Valid`                | 22    | 1     |                                                    |
  | `TAG`                  | 21–0  | 22    |                                                    |
...
- Format II field meanings. [CPU > Format II]

  | Field        | Meaning                                               |
  | ------------ | ----------------------------------------------------- |
  | `imm`        | source value and right-hand operand                   |
  | `cond`       | condition for `SETF`                                  |
  | `regID`      | zero-extended system register index for `LDSR`/`STSR` |
  | `vector`     | zero-extended vector for `TRAP`                       |
  | `sub-opcode` | additional opcode bits for bit string instructions    |

  Whether `imm` is sign-extended depends on the instruction. Whether `imm` is sign-extended depends on the instruction
...
- Interrupts are ignored during exception processing, meaning whenever either `EP` or `NP` is set in `PSW`. [CPU > Exceptions > Interrupt]
...
- Exception list. A dash reproduces the dash the document prints in that cell. [CPU > List of Exceptions]

  | Name                             | Type         | Code              | Level | Handler address | Restore PC     | Note                                      |
  | -------------------------------- | ------------ | ----------------- | ----- | --------------- | -------------- | ----------------------------------------- |
  | Reset                            | Interrupt    | `0xFFF0`          | –     | `0xFFFFFFF0`    | –              | occurs at system power-on                 |
  | Duplexed exception               | Exception    | –                 | –     | `0xFFFFFFD0`    | Current PC     |                                           |
  | VIP                              | Interrupt    | `0xFE40`          | 4     | `0xFFFFFE40`    | see note below | various video conditions                  |
  | Communication                    | Interrupt    | `0xFE30`          | 3     | `0xFFFFFE30`    | see note below | completion of communication port transfer |
  | Game Pak                         | Interrupt    | `0xFE20`          | 2     | `0xFFFFFE20`    | see note below | initiated by the game pak                 |
  | Timer Zero                       | Interrupt    | `0xFE10`          | 1     | `0xFFFFFE10`    | see note below | the timer counter reached zero            |
  | Game Pad                         | Interrupt    | `0xFE00`          | 0     | `0xFFFFFE00`    | see note below | controller button press                   |
  | Address trap                     | Address trap | `0xFFC0`          | –     | `0xFFFFFFC0`    | Current PC     |                                           |
  | `TRAP` (vector < 16)             | Exception    | `0xFFA0 + vector` | –     | `0xFFFFFFA0`    | Next PC        |                                           |
  | `TRAP` (vector ≥ 16)             | Exception    | `0xFFA0 + vector` | –     | `0xFFFFFFB0`    | Next PC        |                                           |
  | Illegal opcode                   | Exception    | `0xFF90`          | –     | `0xFFFFFF90`    | Current PC     |                                           |
  | Zero division                    | Exception    | `0xFF80`          | –     | `0xFFFFFF80`    | Current PC     |                                           |
  | Floating-point reserved operand  | Exception    | `0xFF60`          | –     | `0xFFFFFF60`    | Current PC     |                                           |
  | Floating-point invalid operation | Exception    | `0xFF70`          | –     | `0xFFFFFF60`    | Current PC     |                                           |
  | Floating-point zero division     | Exception    | `0xFF68`          | –     | `0xFFFFFF60`    | Current PC     |                                           |
  | Floating-point overflow          | Exception    | `0xFF64`          | –     | `0xFFFFFF60`    | Current PC     |                                           |
...
- Arithmetic instructions. [CPU > Arithmetic]

  | Mnemonic    | Opcode   | Format | Cycles |
  | ----------- | -------- | ------ | ------ |
  | `ADD` (imm) | `010001` | II     | 1      |
  | `ADD` (reg) | `000001` | I      | 1      |
  | `ADDI`      | `101001` | V      | 1      |
...
- Bitwise instructions. All take 1 cycle and clear `OV` to 0. [CPU > Bitwise]
  ...
  All take 1 cycle and clear `OV` to 0
...
- CPU control instructions. [CPU > CPU Control]

  | Mnemonic | Opcode   | Format | Cycles                 |
  | -------- | -------- | ------ | ---------------------- |
  | `Bcond`  | `100`    | III    |                        |
  | `HALT`   | `011010` | II     | (cycles listed as "–") |
  | `JAL`    | `101011` | IV     | 3                      |
  | `JMP`    | `000110` | I      | 3                      |
  | `JR`     | `101010` | IV     | 3                      |
  | `LDSR`   | `011100` | II     | 8                      |
...
- Floating-point instructions are Format VII sharing opcode `111110`, identified by sub-opcode. [CPU > Floating-Point]

  | Mnemonic  | Sub-opcode | Cycles |
  | --------- | ---------- | ------ |
  | `ADDF.S`  | `000100`   | 9–28   |
  | `CMPF.S`  | `000000`   | 7–10   |
  | `CVT.SW`  | `000011`   | 9–14   |
...
- Floating-point status conditions. [CPU > Floating-Point]

  | Flag  | Code      | Name                  | Condition                                                                                  |
  | ----- | --------- | --------------------- | ------------------------------------------------------------------------------------------ |
  | `FRO` | `0xFF60`  | Reserved operand      | either operand is a NaN, an indefinite, or a non-zero denormal                             |
  | `FIV` | `0xFF70`  | Invalid operation     | a conversion or truncation on a value beyond word range, or `DIVF.S` dividing zero by zero |
  | `FZD` | `0xFF68`  | Zero division         | `DIVF.S` dividing non-zero by zero                                                         |
  | `FOV` | `0xFF64`  | Overflow              |                                                                                            |
  | `FUD` | (no code) | Underflow             | zero is used as the result                                                                 |
  | `FPR` | (no code) | Precision degradation | underflow did not occur and the result was rounded with loss of precision                  |
...
- Bitwise bit string sub-opcodes. None affect status flags. [CPU > Bit Strings > Bitwise]
...
- A per-instruction notation table lists operand order for every mnemonic in the instruction set. The forms it gives are: [CPU > Assembly Notation]

  | Form                                                              | Applies to                                                                                                                                                                                                                                                                             |
  | ----------------------------------------------------------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
  | `MNEM reg1, reg2`                                                 | all Format I register-register operations and all floating-point operations: `ADD`, `AND`, `CMP`, `DIV`, `DIVU`, `MOV`, `MPYHW`, `MUL`, `MULU`, `NOT`, `OR`, `REV`, `SAR`, `SHL`, `SHR`, `SUB`, `XOR`, `ADDF.S`, `CMPF.S`, `CVT.SW`, `CVT.WS`, `DIVF.S`, `MULF.S`, `SUBF.S`, `TRNC.SW` |
  | `MNEM imm, reg2`                                                  | the Format II immediate forms: `ADD`, `CMP`, `MOV`, `SAR`, `SHL`, `SHR`                                                                                                                                                                                                                |
  | `MNEM imm, reg1, reg2`                                            | the Format V three-operand instructions: `ADDI`, `ANDI`, `MOVEA`, `MOVHI`, `ORI`, `XORI`                                                                                                                                                                                               |
  | `MNEM disp[reg1], reg2`                                           | loads and inputs (`LD.B`, `LD.H`, `LD.W`, `IN.B`, `IN.H`, `IN.W`) and `CAXI`                                                                                                                                                                                                           |
  | `MNEM reg2, disp[reg1]`                                           | stores and outputs: `ST.B`, `ST.H`, `ST.W`, `OUT.B`, `OUT.H`, `OUT.W`                                                                                                                                                                                                                  |
  | `JAL address`, `JR address`, `JMP [reg1]`                         | jumps                                                                                                                                                                                                                                                                                  |
  | `LDSR reg2, regID`, `STSR regID, reg2`                            | system register transfer                                                                                                                                                                                                                                                               |
  | `TRAP vector`                                                     | `TRAP`                                                                                                                                                                                                                                                                                 |
  | `XB reg2`, `XH reg2`                                              | `XB` and `XH`                                                                                                                                                                                                                                                                          |
  | `SETF cond, reg2` (V810 notation) or `SETFNZ reg2` (IAR notation) | `SETF`                                                                                                                                                                                                                                                                                 |
  | `BNZ address`                                                     | `BNZ` and the other `Bcond` mnemonics, as one-operand instructions                                                                                                                                                                                                                     |
  | no operands                                                       | `CLI`, `SEI`, `HALT`, `RETI`, `NOP` and every bit string mnemonic                                                                                                                                                                                                                      |
...
- Primary opcode map. [CPU > Opcode Map]
  ...
  | `111100` | VI     | `OUT.B`                           |
  | `111101` | VI     | `OUT.H`                           |
  | `111110` | VII    | floating-point and Nintendo group |
  | `111111` | VI     | `OUT.W`                           |

- Bit string sub-opcode map, opcode `011111`. Sub-opcodes between `00011` and `01000` are Invalid, as are all sub-opcodes above `01111`. [CPU > Opcode Map]
...
- Format VII sub-opcode map, opcode `111110`. All sub-opcodes above `001100` are Invalid. [CPU > Opcode Map]
...
- Communication control field meanings. [Communication Port > Communication Control]

  | Field       | Meaning                                                                                     |
  | ----------- | ------------------------------------------------------------------------------------------- |
  | `C-Int-Inh` | when clear the communication interrupt is enabled. When set it is acknowledged and disabled |
  | `C-Clk-Sel` | 0 = internal clock, 1 = external (another unit's) clock                                     |
  | `C-Start`   | setting begins a communication operation. Always set when read                              |
  | `C-Stat`    | set while communication is underway                                                         |

  when clear the communication interrupt is enabled When set it is acknowledged and disabled Always set when read
...
- A game pak is a cartridge containing a ROM module and optionally a RAM module, a battery, other circuitry, or any combination of those. [Game Pak]
...
- VIP memory map. [VIP > Memory Map]
  ...
  | `0x0005E000`–`0x0005FFFF` | I/O registers                        |
  ...
  | `0x00080000`–`0x00FFFFFF` | mirroring of the VIP memory map      |

- VIP I/O registers. [VIP > Memory Map > I/O Registers]
  ...
  | `0x0005F842`              | `XPCTRL`        |
  | `0x0005F844`              | `VER`           |
  | `0x0005F848`–`0x0005F84E` | `SPT0`–`SPT3`   |
  | `0x0005F860`–`0x0005F866` | `GPLT0`–`GPLT3` |
  | `0x0005F868`–`0x0005F86E` | `JPLT0`–`JPLT3` |
  | `0x0005F870`              | `BKCOL`         |
...
- Object field meanings. [VIP > Objects]

  | Field           | Meaning                                                                                               |
  | --------------- | ----------------------------------------------------------------------------------------------------- |
  | `JX`            | Display Pointer X (signed horizontal coordinate of the object's left edge from the image's left edge) |
  | `JLON`/`JRON`   | draw to left/right image                                                                              |
  | `JP`            | Parallax (signed offset applied to the horizontal coordinate)                                         |
  | `JY`            | Display Pointer Y                                                                                     |
  | `JPLTS`         | Palette Selector                                                                                      |
  | `JHFLP`/`JVFLP` | horizontal/vertical flip                                                                              |
  | `JCA`           | Character Number                                                                                      |

  Unused bits "are not used by the VIP, but are functional memory.".
...
- Six world varieties. [VIP > Worlds > Overview]

  - Normal
  - H-Bias (adds per-row independent horizontal shifting)
  - Affine (per-row source coordinate and vector, allowing rotation, scaling "and can even be used to achieve perspective")
  - Object
  - Dummy (both `LON` and `RON` clear, produces no output)
  - Control (`END` set, signals early termination of the drawing procedure)
...
- World attribute halfword layout. [VIP > Worlds > World Attributes]

  | Halfword | Fields, with bit widths  |
  | -------- | ------------------------ |
  | 1        | unused 6, `GX` 10        |
  | 2        | unused 6, `GP` 10        |
  | 3        | `GY` 16                  |
  ...
  | 8        | `H` 16                   |
  | 9        | `Param Base` 16          |
  | 10       | `Overplane Character` 16 |
  | 11–15    | unused 16                |

- World field meanings. [VIP > Worlds > World Attributes]

  | Field         | Name            | Meaning                                                                                                     |
  | ------------- | --------------- | ----------------------------------------------------------------------------------------------------------- |
  | `LON`, `RON`  |                 | draw to the left and right image respectively                                                               |
  | `BGM`         | BG Modification |                                                                                                             |
  | `SCX`, `SCY`  | Screen X/Y Size | "Raise 2 to this power for the width [height] of the world's background in background maps"                 |
  | `OVER`        | Overplane       | if clear the background repeats indefinitely, if set characters beyond the bounds use `Overplane Character` |
  | `END`         |                 | if set, this world and all worlds of lesser index are not drawn                                             |
  | `BG Map Base` |                 | index of the first background map                                                                           |

- More world fields. Unused bits are "used by the VIP as work memory." [VIP > Worlds > World Attributes]

  | Field                 | Name                    | Meaning                                                                                        |
  | --------------------- | ----------------------- | ---------------------------------------------------------------------------------------------- |
  | `GX`                  | BG X Destination        | signed                                                                                         |
  | `GP`                  | BG Parallax Destination | signed                                                                                         |
  | `GY`                  | BG Y Destination        | signed                                                                                         |
  | `MX`                  | BG X Source             | signed, relative to the background's top-left corner, displayed in the world's top-left corner |
  | `MP`                  | BG Parallax Source      | signed                                                                                         |
  | `MY`                  | BG Y Source             | signed                                                                                         |
  | `W`                   | Window Width            | add 1 for width in pixels                                                                      |
  | `H`                   | Window Height           | add 1 for height in pixels, value is signed                                                    |
  | `Param Base`          |                         |                                                                                                |
  | `Overplane Character` |                         |                                                                                                |
...
- Object worlds ignore all world attributes except `BGM`, which must be 3. `LON`/`RON` do not influence which eyes objects are drawn to, which object attributes determine entirely. If both are clear the world is interpreted as a dummy world and skipped, and "the world does not count as an object world." [VIP > Object Worlds]
...
- Column table entry format. [VIP > Column Table]

  | Field           | Bits | Access | Meaning                                                                       |
  | --------------- | ---- | ------ | ----------------------------------------------------------------------------- |
  | `Repeat`        | 15–8 | R/W    | add 1 for the number of times to produce LED pulses for each column of pixels |
  | `Column Length` | 7–0  | R/W    | add 1 for the emission time per column, in units of 200 nanoseconds           |
...
- Display field meanings. [VIP > Display]

  | Field                           | Access and meaning                                                       |
  | ------------------------------- | ------------------------------------------------------------------------ |
  | `LOCK`                          | (R/W) prevents `CTA` from updating                                       |
  | `SYNCE`                         | (R/W) when clear, display sync signals are not sent to the display servo |
  | `RE`                            | (R/W) when clear, memory refresh signals are not issued on VIP memory    |
  | `FCLK`                          | (R) the display frame clock signal is high                               |
  | `SCANRDY`                       | (R) the mirrors are stable                                               |
  | `R1BSY`/`L1BSY`/`R0BSY`/`L0BSY` | (R) the corresponding frame buffer is being displayed                    |
  | `DISP`                          | (R/W) display enabled                                                    |
  | `DPRST`                         | (W) resets display functions when set                                    |
...
- Drawing field meanings. [VIP > Drawing]

  | Field           | Access and meaning                                                |
  | --------------- | ----------------------------------------------------------------- |
  | `SBOUT`         | (R) set when a group of 8 rows begins to draw                     |
  | `SBCOUNT`       | (R) the group of 8 rows currently being drawn                     |
  | `SBCMP`         | (W) the group of 8 rows to compare with while drawing             |
  | `OVERTIME`      | (R) the drawing procedure has taken longer than the allotted time |
  | `F1BSY`/`F0BSY` | (R) which frame buffer is being drawn to                          |
  | `XPEN`          | (R/W) drawing enabled                                             |
  | `XPRST`         | (W) resets drawing functions when set                             |
  | `F1BSY`/`F0BSY` | "are formally sub-fields of a 2-bit field called `XPBSY`."        |
...
- Interrupt condition meanings. [VIP > Interrupts]

  | Condition    | Meaning                                                                                            |
  | ------------ | -------------------------------------------------------------------------------------------------- |
  | `TIMEERR`    | drawing still in progress when the drawing procedure should begin (detects `OVERTIME` in `XPSTTS`) |
  | `XPEND`      | the drawing procedure has finished                                                                 |
  | `SBHIT`      | drawing has begun on the group of 8 rows specified in `SBCMP` of `XPCTRL`                          |
  ...
  | `LFBEND`     | completed for the left eye                                                                         |
...
- VSU memory map. [VSU > Memory Map]
  ...
  | `0x01000400`–`0x010007FF` | I/O registers                   |
  | `0x01000800`–`0x01FFFFFF` | mirroring of the VSU memory map |
...
- Writing `SxINT` resets the following. [VSU > Channel Control]

  - the frequency delay counter to the beginning of its current sample
  - The current position in PCM wave memory to the first sample
  - The envelope step timer to the start of its step interval
  - The frequency modification timer to the start of its modification interval
  - The current position in modulation memory to the first value
  - The noise generator's shift register to all zeroes
